...
step_services() {
    ...
    if prompt_yes_no "Enable Redis (cache, sessions & required for Celery)?" "n"; then
        USE_REDIS="true"
        ...
        if prompt_yes_no "Enable Celery (background tasks)? Requires Redis." "n"; then
            ...
            USE_CELERY="false"
            printf "  ${DIM}  Celery: OFF${RESET}\n"
        fi
    else
        USE_CELERY="false"
        printf "  ${DIM}  Redis: OFF${RESET}\n"
        printf "  ${DIM}  Celery: OFF (Requires Redis)${RESET}\n"
    fi
...
apply_finish() {
    ...
    if [ "$RUN_MODE" = "docker" ]; then
        ...
        box_empty
        box_line "  docker compose up --build -d"
        box_empty
